...
public class BlockTagGenerator extends VanillaBlockTagsProvider {
    ...
    protected void generateInfinityButtonsTags() {
        ...
        for (var entry : IBBlocks.SECRET_BUTTONS.values()) {
            ...
            ResourceLocation location = BuiltInRegistries.BLOCK.getKey(entry.get());
    ...
    protected void generateVanillaTags() {
        ...
        for (var entry : IBBlocks.SECRET_BUTTONS.values()) {
            if (!IBModdedBlocks.MODDED_BUTTONS.containsKey(entry)) {
                this.tag(MINEABLE_PICKAXE).add(entry.get());
                continue;
            }
            ResourceLocation location = BuiltInRegistries.BLOCK.getKey(entry.get());
            this.tag(MINEABLE_PICKAXE).addOptional(location);
        }

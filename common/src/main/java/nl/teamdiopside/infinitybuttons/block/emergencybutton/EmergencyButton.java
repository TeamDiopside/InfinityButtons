...
import nl.teamdiopside.infinitybuttons.block.button.AbstractButton;
import nl.teamdiopside.infinitybuttons.config.AlarmEnum;
...
import static nl.teamdiopside.infinitybuttons.InfinityButtons.ALARM_PACKET;
...
public class EmergencyButton extends AbstractButton {
    ...
    public EmergencyButton(Settings settings) {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (!world.isClient && InfinityButtons.CONFIG.alarmVillagerPanic()) {
            List<LivingEntity> villagers = new ArrayList<>();
            if (InfinityButtons.CONFIG.alarmSoundType() == AlarmEnum.GLOBAL) {
            ...
            if (InfinityButtons.CONFIG.alarmSoundType() == AlarmEnum.RANGE) {
                villagers = world.getEntitiesByClass(LivingEntity.class, new Box(pos).expand(InfinityButtons.CONFIG.alarmSoundRange()), entity -> entity.getType() == EntityType.VILLAGER);
            }
    ...
    public static void emergencySound(World world, BlockPos pos) {
        if (!world.isClient) {
            ...
            buf.writeEnumConstant(InfinityButtons.CONFIG.alarmSoundType());
